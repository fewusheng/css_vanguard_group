...
#  CSS3  
##  CSS3选择器 -- 属性选择器（1）
E[attr="value"]指定属性名，并指定了该属性的值  
E[attr~="value"]指定属性名，并具有属性值，此属性是一个词列表，  
并且以空格隔开，其中词列表中包含的一个value词，而且等号前面的"~"不能不写  
E[attr^="value"]指定属性名，并具有属性值，属性值是以value开头的  
E[attr$='value']指定属性名，并且有属性值，而且属性值是以value结束的  
E[attr*="value"]指定属性名，并且有属性值，而且属性值中包含了value  
E[attr|="value"]指定属性名，并且有属性值，而且属性值是value或者以"value-"开头的值（如zh-cm）  
###  备注：IE7及以上支持  
